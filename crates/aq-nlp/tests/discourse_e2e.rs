...
fn e2e_technical_report() {
    ...
    assert!(types.len() >= 5, "Expected at least 5 discourse relations in technical report, got {}: {:?}", types.len(), types);
...
fn e2e_argument_essay() {
    ...
    let types = run_query_node_types(text, "desc:discourse");
    assert!(types.len() >= 5, "Expected at least 5 discourse relations in essay, got {}: {:?}", types.len(), types);
}
...
fn e2e_no_connectives() {
    ...
    assert!(types.is_empty(), "Expected no discourse for connective-free text, got: {:?}", types);
}
...
fn e2e_dense_connectives() {
    ...
    let types = run_query_node_types(text, "desc:discourse");
    assert_eq!(types.len(), 5, "Expected 5 discourse relations in dense connective text, got {}: {:?}", types.len(), types);
}
...
fn e2e_discourse_plus_entities_plus_roles() {
    ...
    assert_eq!(discourse.len(), 2, "Expected 2 discourse relations, got: {:?}", discourse);
...
fn e2e_cross_paragraph_multi() {
    ...
    let types = run_query_node_types(text, "desc:discourse");
    assert!(types.len() >= 3, "Expected at least 3 discourse relations, got: {:?}", types);
    // Check cross-paragraph scope exists
    let scopes = run_query(text, "desc:discourse | .scope");
    assert!(scopes.iter().any(|s| s == "cross_paragraph"), "Expected at least one cross_paragraph relation, got: {:?}", scopes);
}
