...
fn adr_evidence_query() {
    ...
    assert_eq!(
        results.len(),
        1,
        "Expected 1 Evidence relation, got: {:?}",
        results
    );
}
...
fn adr_contrast_query() {
    ...
    assert_eq!(
        results.len(),
        2,
        "Expected 2 Contrast relations, got: {:?}",
        results
    );
...
fn adr_condition_query() {
    ...
    assert!(
        !results.is_empty(),
        "Expected at least 1 Condition relation, got: {:?}",
        results
    );
}
...
fn adr_nucleus_line_query() {
    ...
    assert_eq!(
        results.len(),
        1,
        "Expected 1 relation with nucleus on line 1, got: {:?}",
        results
    );
}
...
fn connective_filter_query() {
    ...
    assert_eq!(
        results.len(),
        1,
        "Expected 1 result for connective=However, got: {:?}",
        results
    );
}
...
fn desc_discourse_enumeration() {
    ...
    assert_eq!(
        types.len(),
        2,
        "Expected 2 discourse nodes, got: {:?}",
        types
    );
...
fn discourse_sequence_query() {
    ...
    assert_eq!(
        types.len(),
        2,
        "Expected 2 Sequence relations (Then, Finally), got: {:?}",
        types
    );
}
...
fn tier1_regression_entity() {
    ...
    let types = run_query_node_types("Sarah arrived. However, Bob left.", "desc:entity");
...
fn no_discourse_single_sentence() {
    ...
    let types = run_query_node_types("The server is running.", "desc:discourse");
    assert!(
        types.is_empty(),
        "Expected no discourse for single sentence, got: {:?}",
        types
    );
}
