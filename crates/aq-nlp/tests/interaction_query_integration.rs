...
fn query_all_interactions() {
    ...
    assert!(
        types.len() >= 2,
        "Expected at least 2 interactions, got: {:?}",
        types
    );
...
fn query_all_verb_phrases() {
    ...
    let types = run_query_node_types("Sarah ran. The cat slept.", "desc:verb_phrase");
    assert!(
        types.len() >= 2,
        "Expected at least 2 verb_phrases, got: {:?}",
        types
    );
...
fn query_passive_interaction_has_correct_agent() {
    ...
    assert!(
        ...
        "Passive voice agent should be Sarah, got: {:?}",
        agent_texts
    );
}
...
fn query_interaction_pipe_agent() {
    ...
    assert!(
        joined.contains("Sarah") || joined.contains("sarah"),
        "Should find Sarah: {:?}",
        texts
    );
    assert!(
        joined.contains("Bob") || joined.contains("bob"),
        "Should find Bob: {:?}",
        texts
    );
}
...
fn query_verb_phrase_voice() {
    ...
    let texts = run_query("The cat was chased by Sarah.", "desc:verb_phrase | .voice");
    assert!(
        texts.contains(&"passive".to_string()),
