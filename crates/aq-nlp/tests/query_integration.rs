...
fn query_all_entities() {
    ...
    assert!(results.iter().all(|t| t == "entity"), "All results should be entities: {:?}", results);
}
...
fn query_person_entities() {
    ...
    assert_eq!(results.len(), 2, "Expected 2 PERSON entities, got: {:?}", results);
}
...
fn query_gpe_entities() {
    ...
    assert_eq!(results.len(), 2, "Expected 2 GPE entities, got: {:?}", results);
}
...
fn query_sentences() {
    ...
    assert!(results.iter().all(|t| t == "sentence"), "All results should be sentences: {:?}", results);
}
...
fn query_noun_tokens() {
    ...
    assert_eq!(results.len(), 2, "Expected 2 NOUN tokens (cat, mat), got: {:?}", results);
}
...
fn query_entity_text() {
    ...
    assert!(results.contains(&"Sarah".to_string()), "Expected 'Sarah' in {:?}", results);
    assert!(results.contains(&"Paris".to_string()), "Expected 'Paris' in {:?}", results);
}
...
fn query_on_empty_document() {
    ...
    assert!(results.is_empty(), "Expected no entities for empty doc, got: {:?}", results);
}
...
fn query_verb_tokens() {
    ...
    assert_eq!(results.len(), 1, "Expected 1 VERB token (chased), got: {:?}", results);
    assert!(results.contains(&"chased".to_string()), "Expected 'chased' in {:?}", results);
}
